...
from hf_integration_sample.errors.types import ErrorType


class Location(BaseModel):
    entity: str = Field("body", description="Entity where error was raised", example="body")
    variable: str = Field(None, description="Json-pointer path to error", example="/users/1/name")


class ErrorBase(BaseModel):
    type: str = Field(..., description="Error type", example="type_error.integer")
    title: str = Field(..., description="Title", example="value is not a valid integer")
    detail: t.Optional[str] = Field(
        None,
        description="Error detail",
        example="Expected valid integer, got: 'peanut'",
    )
